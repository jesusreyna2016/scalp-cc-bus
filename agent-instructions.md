...
5c. **SL de 3 capas vs SL de vela 1 del FVG** (`sl_origin_vs_layer`): medición
   PARALELA con misma entrada y mismos TP; `rMultiple` es en base al SL de 3 capas,
   `rOrig` en base al SL puesto en la vela 1 (origen) del FVG (long = su mínimo,
   short = su máximo, crudo, sin piso/techo). Mira `delta_orig_minus_layer` y su
   `delta_ci90`: sólo trátalo como real si `delta_beats_zero` o `delta_below_zero`
   (CI90 no cruza 0) con n>=30. `orig_saved_from_SL` / `orig_caused_SL` = cuántos
   stops se evitan o se provocan al mover el SL. Este SL NO se aplica todavía: si
   gana de forma estable (FDR + walk-forward), pasa a `experiments.json` /
   propuesta de la revisión semanal, no antes.
